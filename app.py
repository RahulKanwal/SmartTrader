...
def make_predictions(data, high_model, low_model, avg_close_model, open_model, user_date):
    ...
    if len(last_data) == 5:
        ...
        for i, future_date in enumerate(future_dates):
            ...
            print(f" Predicted Open: {open_pred[i]:.2f}")
...
def perform_additional_functions(data, high_pred, low_pred, avg_close_pred, open_pred, user_date):
    ...
    highest_price = round(np.max(high_predictions), 2)
    lowest_price = round(np.min(low_predictions), 2)
    avg_closing_price = round(np.mean(avg_close_predictions), 2)
    avg_opening_price = round(np.mean(open_predictions), 2)
    ...
    # headers = ['Date', 'Action', 'NVDA shares', 'NVDQ shares', 'Portfolio Value']
...
def perform_predictions(user_date):
    ...
    if high_pred is not None and low_pred is not None and avg_close_pred is not None and open_pred is not None:
        results = perform_additional_functions(data, high_pred, low_pred, avg_close_pred, open_pred, user_date)
        return results
